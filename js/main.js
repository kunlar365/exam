...
function filterMoviesByGen(genre) {
...
const genButtonsEl = document.getElementsByClassName('sidebar')[0];

genButtonsEl.addEventListener('click', function (event) {
    if (event.target.className == 'genre__item') {
        let selectedGen = event.target.innerText;
        let filteredMovies;
        if (selectedGen !== 'All') {
            filteredMovies = filterMoviesByGen(selectedGen);
        } else {
            filteredMovies = movies;
        }

        moviesEl.innerHTML = '';
        appendSelectedMovies(filteredMovies);
    }
});
...
function searchMovie(searchVideo) {
    let resultArr = movies.filter((el) => {
        return el.title.toLowerCase().includes(searchVideo)
    });
    return resultArr
}


const searchFilm = document.getElementsByClassName('search__input')[0];

searchFilm.addEventListener('keyup', function () {
    let searchResult = searchMovie(searchFilm.value);
    moviesEl.innerHTML = '';
    appendSelectedMovies(searchResult);
});


let seeLater = cartItemsFromLocalStorage();

function addMovieToMyFav(movieId) {
    if (!seeLater.includes(movieId)) {
        seeLater.push(movieId);
    }
}
...
if (moviesEl) {
    moviesEl.addEventListener('click', function (event) {
        if (event.target.className == 'movie__btn') {
            let idOfTouchedMovie = +event.target.getAttribute('data-id');
            addMovieToMyFav(idOfTouchedMovie);
...
function cartItemsToLocalStorage() {
    localStorage.setItem('meningVideolarim', JSON.stringify(seeLater))
}


function cartItemsFromLocalStorage() {
    let result = JSON.parse(localStorage.getItem('meningVideolarim'));
    return result ? result : [];
}


function createCartItemForFav(el) {
    ...
    <img src="img/${el.poster}" class="movie__poster">
                        <div class="movie__info">
                            <h2 class="movie__title">${el.title}</h2>
                            <h3 class="movie__genre">${el.genres}</h3>
                            <button class="delete" type="button" data-id="${el.id}">Remove from watchlist</button>
                        </div>
    `
    return movieEl;
}
...
const laterItemsEl = document.getElementById('watchlistMovie');
...
function moviesInCart() {
    laterItemsEl.innerHTML = '';
    let favFilmsArr = cartItemsFromLocalStorage();
    favFilmsArr.forEach((el) => {
        let movie = movies.filter((event) => {
            return event.id == el
        })
        let element = createCartItemForFav(movie[0]);
        laterItemsEl.appendChild(element);
    })
}


if(laterItemsEl) {
    laterItemsEl.addEventListener('click', function(event) {
        if(event.target.className == 'delete') {
            let elId = event.target.getAttribute('data-id') 
            let favFilmsArr = cartItemsFromLocalStorage();
            
            favFilmsArr.splice(favFilmsArr.indexOf(+elId), 1);
    
            localStorage.setItem('meningVideolarim', JSON.stringify(favFilmsArr));
            moviesInCart();
        }
    })
}